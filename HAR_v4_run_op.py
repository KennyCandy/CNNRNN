...
if __name__ == "__main__":
    ...
    print("Loading data...")
    X_train_op, y_train_op, X_test_op, y_test_op = load_dataset('data/oppChallenge_gestures_2.data')
    ##########################################
...
if (FLAG == 'train') :
    with tf.Session() as sess:
        ...
        sess.run(init)  # .run()
        writer = tf.train.SummaryWriter("opprboard", graph=tf.get_default_graph())
